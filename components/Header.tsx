...
export const Header = () => {
  ...
  return (
    <>
      <div style={{ backgroundColor: "var(--bg-color)" }} className='w-full h-4'></div>
      <nav style={{ backgroundImage: "radial-gradient(circle,rgba(16 18 27 / 30%), #27272a" }} className=" w-full m-auto bg-zinc-800 shadow-bottom shadow-zinc-900">
        ...
        <div className="md:hidden border-t border-zinc-400">
          {showMobileMenu && <Menu />}
        </div>
      </nav>
    </>
  ...
  return <div className="px-2 md:px-0 py-3 space-y-2 md:space-y-0 md:space-x-2">
    {navigationLinks.map((link, index) => {
      ...
      return <Link
        ...
        className={` text-center block md:inline-block px-3 py-2 items-center  text-purple-500  hover:font-extrabold hover:text-2xl ${isActive ? "font-extrabold text-2xl from-purple-600 via-pink-600 to-blue-600 bg-gradient-to-r background-animate bg-clip-text text-transparent" : ""}`}
      >
        {link.title}
      </Link>
    })}
